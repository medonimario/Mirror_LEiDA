...
This version uses a paired permutation test for statistical inference, which is
a powerful and distribution-free alternative to the Wilcoxon test.
"""
...
# --- STATISTICAL ANALYSIS FUNCTION (UPDATED) ---

def permutation_test_paired(x, y, n_permutations=10000, seed=None):
    """
    Performs a non-parametric paired permutation test.
    
    Args:
        x, y (array-like): Paired data for two conditions.
        n_permutations (int): Number of permutations to generate the null distribution.
        seed (int): Random seed for reproducibility.

    Returns:
        float: The observed mean difference (statistic).
    ...
    for i in range(n_permutations):
        ...
        swap = rng.choice([False, True], size=len(x))
        
        permuted_x = np.where(swap, all_data[:, 1], all_data[:, 0])
        permuted_y = np.where(swap, all_data[:, 0], all_data[:, 1])
        
        null_distribution[i] = np.mean(permuted_x - permuted_y)
...
def perform_all_tests(clustering_base_dir: Path, k_min: int, k_max: int, conditions: list[str], n_permutations: int, seed: int):
    """
    Loops through all K, loads data, and performs permutation tests,
    ...
    for k in range(k_min, k_max + 1):
        ...
        for cond1, cond2 in combinations(conditions, 2):
            for state_name in sorted(fo_df['state'].unique()):
                ...
                if merged.shape[0] >= 5:
                    test_seed = rng_master.integers(1e9) # New seed for each test
                    stat, p_raw = permutation_test_paired(merged['FO_x'], merged['FO_y'], n_permutations, test_seed)
                
                all_fo_results.append({
                    'K': k, 'state': state_name, 'comparison': f"{cond1}_vs_{cond2}",
                    'metric': 'FO', 'statistic (mean_diff)': stat, 'p_value_raw': p_raw
                })
        ...
        for cond1, cond2 in combinations(conditions, 2):
            for state_name in sorted(dt_df['state'].unique()):
                ...
                if merged.shape[0] >= 5:
                    test_seed = rng_master.integers(1e9) # New seed for each test
                    stat, p_raw = permutation_test_paired(merged['DT_s_x'], merged['DT_s_y'], n_permutations, test_seed)

                all_dt_results.append({
                    'K': k, 'state': state_name, 'comparison': f"{cond1}_vs_{cond2}",
                    'metric': 'DT_s', 'statistic (mean_diff)': stat, 'p_value_raw': p_raw
                })
...
def main():
    parser = argparse.ArgumentParser(
        description="Run non-parametric statistics (paired permutation test) with GLOBAL FDR correction.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    # --- Path-building Arguments ---
    ...
    parser.add_argument('--n_permutations', type=int, default=10000, help="Number of permutations for the test.")
    ...
    print("--- Global Statistical Analysis (Permutation Test) Started ---")
    print(f"Reading from: {clustering_base_dir.resolve()}")
    
    # 1. Gather all raw p-values from all K's
    print(f"\nStep 1: Gathering raw p-values ({args.n_permutations} perms/test) from all K directories...")
    fo_results_raw, dt_results_raw = perform_all_tests(
        clustering_base_dir, args.k_min, args.k_max, args.conditions, args.n_permutations, args.seed
    )
    ...
    fo_output_path = clustering_base_dir / "STATS_GLOBAL_FO_perm.csv"
    fo_results_corrected.to_csv(fo_output_path, index=False, float_format='%.5f')
    print(f"  -> Saved FO results to: {fo_output_path}")

    dt_output_path = clustering_base_dir / "STATS_GLOBAL_DT_perm.csv"
